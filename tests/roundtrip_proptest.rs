...
use scim_v2::{Asserted, ScimDateTime};
...
/// An `Asserted<T>` across all three states. A round-trip must return the
/// same state, so the property covers the RFC 7644 §3.5.1 distinction rather
/// than only the values.
fn asserted<S>(inner: S) -> impl Strategy<Value = Asserted<S::Value>>
where
    S: Strategy,
    S::Value: std::fmt::Debug + Clone + 'static,
{
    prop_oneof![
        Just(Asserted::Absent),
        Just(Asserted::Nulled),
        inner.prop_map(Asserted::Set),
    ]
}
